...
class RlDataGenerator:
    ...
    def extract_number(self, filename):
        # 自定义排序函数，提取文件名中的数字并根据这个数字排序
        match = re.search(r'\d+', filename)  # 正则表达式匹配数字
    ...
    def batch_generate_rl_data(self):
        ...
        # 根据提取的数字对文件列表进行排序
        csv_files = sorted(csv_files, key=self.extract_number)
        print(csv_files)
        training_data_list = []
...
def generate_rl_data():
    ...
    file_folder_path = "/mnt/f/competion/traffic"
